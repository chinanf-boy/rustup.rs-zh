...
- [处理窗户上的锈迹](#%E5%A4%84%E7%90%86%E7%AA%97%E6%88%B7%E4%B8%8A%E7%9A%84%E9%94%88%E8%BF%B9)
...
又许多`rustup`处理*工具链*的命令，Rust 编译器的单个安装。`rustup`支持多种类型的工具链。官方发布版本最基本的轨道：_稳定_，*测试*和*每夜*；但`rustup`还可以从官方存档，备用主机平台和本地构建中，安装工具链。
...
企业网络通常不具有直接的外部 HTTP 访问，但强制使用代理。如果您在这样的网络上，您可以通过在环境中设置其 URL 来请求使用代理。在大多数情况下，设置`https_proxy`应该足够了。在类似 Unix 的系统上**bash**或**zsh**，您可以使用：
...
如果需要更复杂的设置，rustup 支持**curl**程序，记录在环境部分[其手册页][curlman].

使用`curl`目前**已弃用**但是，它仍然可以通过提供`RUSTUP_USE_CURL`环境变量，例如：

```bash
RUSTUP_USE_CURL=1 rustup update
```

注意有些版本的`libcurl`显然是要求你放弃`http://`或`https://`环境变量中的前缀。例如，`export http_proxy=proxy.example.com:1080`（同样适用于 HTTPS）。如果您得到一个 SSL`unknown protocol`来自的错误`rustup`通过`libcurl`但是命令行`curl`命令工作正常，这可能是问题所在。
...
| `rustup override set nightly-2015-04-01`                   | 对于当前目录，从特定日期开始每夜使用               |
| `rustup toolchain link my-toolchain "C:\RustInstallation"` | 通过符号链接现有安装来安装自定义工具链             |
| `rustup show`                                              | 显示将在当前目录中使用的工具链                     |
| `rustup toolchain uninstall nightly`                       | 卸载给定的工具链                                   |
| `rustup toolchain help`                                    | 显示`help`子命令的页面（如`toolchain`）            |
| `rustup man cargo`                                         | （_仅限于 Unix_）查看给定命令的手册页（如`cargo`） |

## 环境变量

- `RUSTUP_HOME`（默认：`~/.rustup`或`%USERPROFILE%/.rustup`）设置根 Rust 文件夹，用于存储已安装的工具链和配置选项。

- `RUSTUP_TOOLCHAIN`（默认：无）如果设置，将覆盖用于所有 Rust 刀具调用的工具链。应安装具有此名称的工具链，否则调用将失败。

- `RUSTUP_DIST_SERVER`（默认：`https://static.rust-lang.org`）设置用于下载与 rust 相关的静态资源的根 URL。您可以将其更改为使用本地镜像，或者从临时目录测试二进制文件。

- `RUSTUP_DIST_ROOT`（默认：`https://static.rust-lang.org/dist`）已弃用。使用`RUSTUP_DIST_SERVER`相反。

- `RUSTUP_UPDATE_ROOT`（默认`https://static.rust-lang.org/rustup`）设置下载自我更新的根 URL。

- `RUSTUP_IO_THREADS` _不稳定的_（默认为报告的 CPU 计数）。设置执行关闭 IO 的线程数。设置为`disabled`强制单线程 IO 进行故障排除，或强制任意数字覆盖自动检测。

- `RUSTUP_TRACE_DIR` _不稳定的_（默认：无跟踪）启用跟踪并确定跟踪也将写入的目录。记录道的形式为 pid.trace。弹射器项目可以读取痕迹[跟踪查看器][tv].

  [电视]：（<https://github.com/catapult-project/catapult/blob/master/tracing/README.md>）

- `RUSTUP_UNPACK_RAM` _不稳定的_（默认为 400 米，最小为 100 米）限制了在解包时用于 IO 任务的 RAM Rust 量。

## 其他安装方法

主要安装方法，如中所述<https://rustup.rs>，因平台而异：

- 在 Windows 上，下载并运行[为生成的 rustup-init.exe`i686-pc-windows-gnu`目标][setup]. 一般来说，这是一个 Rust 的建筑，应该安装在窗户上。尽管是针对 GNU 工具链构建的，_如果检测到安装了 msvc，则由 rustup 生成的 Windows 将为 msvc 工具链安装 rust。_. 如果您喜欢安装 gnu toolchains 或 x86_toolchains，默认情况下，可以在安装时以交互方式或使用`--default-host`标志，或安装后通过`rustup set default-host`.
- 在 Unix 上，运行`curl https://sh.rustup.rs -sSf | sh`在你的壳里。此下载和运行[`rustup-init.sh`]从而下载并运行正确版本的`rustup-init`平台的可执行文件。

[setup]: https://static.rust-lang.org/rustup/dist/i686-pc-windows-gnu/rustup-init.exe
[`rustup-init.sh`]: https://static.rust-lang.org/rustup/rustup-init.sh

`rustup-init`接受可以通过 shell 脚本传递的参数。一些例子：
...
如果您愿意，可以直接下载`rustup-init`对于您选择的平台：
...
<a name="vs2019">γ</a>MSVC 构建`rustup`另外需要[VisualStudio 2019 或 VisualC++生成工具 2019 的安装][vs]. 对于 VisualStudio，请确保选中“C++工具”选项。对于 GNU 构建的基本使用，不需要额外的软件安装。

[vs]: https://visualstudio.microsoft.com/downloads/

您可以从中获取旧版本`https://static.rust-lang.org/rustup/archive/{rustup-version}/{target-triple}/rustup-init[.exe]`

要从源安装，只需运行`cargo run --release`. 请注意，目前 Rust 仅在夜间 Rust 的基础上进行，安装后，Rust 的工具链将通过预处理取代任何现有的工具链。`~/.cargo/bin`到`PATH`环境变量。

## 安全

`rustup`对非偏执狂来说足够安全，但是[还需要工作][s]. `rustup`通过 HTTPS 执行所有下载，但尚未验证下载的签名。

[s]: https://github.com/rust-lang/rustup.rs/issues?q=is%3Aopen+is%3Aissue+label%3Asecurity

从 1.18.4 开始，安装荣誉 umask 上的文件模式，如果需要非常严格的控制，请使用 umask。

## 常见问题解答

### 这是官方的铁锈项目吗？

对。Rust 是一项官方的 Rust 工程。建议在<https://www.rust-lang.org>.

### 这与多重信任有什么关系？

Rustup 是[多重 Rust]. Rustup 从 multirst rs 开始，它是 multirst 从 shell 脚本到 rust 的覆盖。[迪戈里·布莱克]现在由 Rust 项目维护。

### Rustup 可以下载 Rust 源代码吗？

可通过运行获得锈源。`rustup component add rust-src`. 它将被下载到`<toolchain root>/lib/rustlib/src/rust`当前工具链的目录。

### Rust 失败，出现 Windows 错误 32
...
### 我得到“错误：无法删除”rustup bin“文件：'c:\\users\\user\\.cargo\\bin\\rustup.exe'“
